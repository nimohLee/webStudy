...
app.get("/main", (req, res) => {
  ...
  conn.query(sql1, (error, results) => {
    if(error) return console.log(error);
      ...
      res.render("main", {title: 'Welcome Pages', company: results});
    });
});
...
app.get("/dbCreate", (req, res) => {
  ...
  let sql2 = "CREATE TABLE small_category(";
    ...
    let sql3 = "CREATE TABLE products(";  
        ...
        sql3 += "productAmount int default 0,",
        sql3 += "productPrice int default 0,";
...
app.get("/dbDrop", (req, res) => {
  ...
  conn.query(sql1 + sql2 + sql3, (error, results) => {
    if(error) return console.log(error);
      ...
      console.log("데이터베이스 > 테이블이 삭제되었습니다.");
      res.redirect("/");
  });
});
...
app.get("/smallCategory", (req, res) => {
  ...
  conn.query(sql, [req.query.bcCode], (error, results) => {
    if(error) return console.log(error);
      ...
      res.render("smallCategory", {title: 'Products Small Category', smallCategory: results});
    });
});

app.get("/productList", (req, res) => {
  let sql = "SELECT * FROM products WHERE substring(productCode, 1,3) = ? AND substring(productCode, 4,3) = ?;";
  conn.query(sql, [req.query.bcCode, req.query.scCode], (error, results) => {
    if(error) return console.log(error);
    else
    res.render("productList", {title: 'Home > 상품목록보기', productList: results});
    });
});
...
app.post("/productAdd", (req, res) => {
  ...
  conn.query(seq, (error, maxPid) => {
    ...
    conn.query(sql, [pCode, req.body.productName, req.body.productAmount, req.body.productPrice], (err, results) => {
      if(error) return console.log(error);
      else
        res.render("index", {title: 'Admin Pages', tblExists: 1});
      });
  });
